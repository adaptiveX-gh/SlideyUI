...
const TEMPLATE_METADATA: Record<SlideType, TemplateMetadata> = {
  ...
  grid: {
    ...
    description: 'Flexible grid layout for feature showcases, product grids, etc.',
    category: 'Layout',
    requiredFields: ['items'],
    optionalFields: ['title', 'subtitle', 'gridType', 'gap'],
    schema: {
      type: '"grid"',
      title: 'string (optional)',
      subtitle: 'string (optional)',
      gridType: '"2x2" | "3x3" | "2x3" | "4x2" | "auto" (optional)',
      items: 'GridItem[]',
      gap: '"compact" | "normal" | "spacious" (optional)',
    },
    example: {
      type: 'grid',
  ...
  'feature-cards': {
    ...
    description: 'Card-based layout optimized for feature showcases',
    category: 'Layout',
    requiredFields: ['features'],
    optionalFields: ['title', 'subtitle', 'columns', 'gap'],
    schema: {
      type: '"feature-cards"',
      title: 'string (optional)',
      subtitle: 'string (optional)',
      features: 'FeatureCardItem[]',
      columns: '"2" | "3" | "4" | "auto" (optional)',
      gap: '"compact" | "normal" | "spacious" (optional)',
    },
    example: {
      type: 'feature-cards',
      title: 'Key Features',
      features: [
        { title: 'Fast', description: 'Lightning-fast performance', highlight: true },
  ...
  team: {
    type: 'team',
    displayName: 'Team Slide',
    description: 'Showcase team members with photos, roles, bios, and social links',
    category: 'Content',
    requiredFields: ['members'],
    optionalFields: ['title', 'layout'],
    schema: {
      type: '"team"',
      title: 'string (optional)',
      members: 'TeamMember[]',
      layout: '"grid" | "carousel" | "highlight" (optional)',
    },
    example: {
      type: 'team',
      title: 'Meet Our Team',
  ...
  pricing: {
    ...
    description: 'Display pricing plans in a comparison-friendly format',
    category: 'Data & Metrics',
    requiredFields: ['plans'],
    optionalFields: ['title', 'highlight'],
    schema: {
      type: '"pricing"',
      title: 'string (optional)',
      plans: 'PricingPlan[]',
      highlight: 'number (optional)',
    },
    example: {
      type: 'pricing',
  ...
  code: {
    ...
    description: 'Display code with syntax highlighting and line numbers',
    category: 'Content',
    requiredFields: ['language', 'code'],
    optionalFields: ['title', 'highlights', 'filename', 'theme'],
    schema: {
      type: '"code"',
      title: 'string (optional)',
      language: 'string',
